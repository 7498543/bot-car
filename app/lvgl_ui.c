...
esp_err_t lvgl_ui_init(void)
{
    ...
    lv_obj_set_style_text_color(s_label_voltage, lv_color_hex(0x00FF00), 0);
    lv_obj_align(s_label_voltage, LV_ALIGN_LEFT_MID, 5, 0);

    /* WiFi 信号标签 */
    s_label_wifi = lv_label_create(status_bar);
    lv_label_set_text(s_label_wifi, "---");
    lv_obj_set_style_text_color(s_label_wifi, lv_color_hex(0x00BFFF), 0);
    lv_obj_align(s_label_wifi, LV_ALIGN_RIGHT_MID, -5, 0);

    /* 模式显示 */
    s_label_mode = lv_label_create(lv_scr_act());
    lv_label_set_text(s_label_mode, "REMOTE");
    lv_obj_set_style_text_color(s_label_mode, lv_color_hex(0xFFFFFF), 0);
    lv_obj_align(s_label_mode, LV_ALIGN_TOP_MID, 0, 40);

    /* 速度条 */
    s_bar_speed = lv_bar_create(lv_scr_act());
    ...
    /* 距离显示 */
    s_label_distance = lv_label_create(lv_scr_act());
    lv_label_set_text(s_label_distance, "Dist: ---cm");
    lv_obj_set_style_text_color(s_label_distance, lv_color_hex(0xCCCCCC), 0);
    lv_obj_align(s_label_distance, LV_ALIGN_TOP_MID, 0, 110);

    /* OTA 进度 */
    s_label_ota = lv_label_create(lv_scr_act());
    lv_label_set_text(s_label_ota, "OTA: idle");
    lv_obj_set_style_text_color(s_label_ota, lv_color_hex(0x888888), 0);
...
void lvgl_ui_update_voltage(uint16_t voltage_mv)
{
    ...
    /* 根据电压设置颜色 */
    if (voltage_mv < 3300) {
        lv_obj_set_style_text_color(s_label_voltage, lv_color_hex(0xFF0000), 0);  /* 红色 */
    } else if (voltage_mv < 3500) {
        lv_obj_set_style_text_color(s_label_voltage, lv_color_hex(0xFFAA00), 0);  /* 橙色 */
    } else {
        lv_obj_set_style_text_color(s_label_voltage, lv_color_hex(0x00FF00), 0);  /* 绿色 */
    }
...
void lvgl_ui_update_mode(car_mode_t mode)
{
    ...
    case CAR_MODE_REMOTE:
        lv_label_set_text(s_label_mode, "REMOTE");
        lv_obj_set_style_text_color(s_label_mode, lv_color_hex(0x00BFFF), 0);
        break;
    case CAR_MODE_TRACK:
        lv_label_set_text(s_label_mode, "TRACK");
        lv_obj_set_style_text_color(s_label_mode, lv_color_hex(0x00FF00), 0);
        break;
    case CAR_MODE_AVOID:
        lv_label_set_text(s_label_mode, "AVOID");
        lv_obj_set_style_text_color(s_label_mode, lv_color_hex(0xFFAA00), 0);
        break;
    }
}
...
void lvgl_ui_update_distance(uint16_t distance_cm)
{
    ...
    char buf[24];
    snprintf(buf, sizeof(buf), "Dist: %ucm", distance_cm);
